...
<!-- Page title -->
                                    ...
                                    <div class="add-btn">
                                        <a href="{{ url('/dashboard/admins/create') }}" type="button" class="btn btn-custom btn-rounded waves-effect waves-light w-md m-b-5"> <i class="fa fa-check"></i> اضافة </a>
                                    </div>
@endsection
@section('content')
                                                    ...
                                                    <table id="datatable-buttons" class="table table-striped table-bordered">
                                                        ...
                                                        <tbody>
                                                            ...
                                                            <tr>
                                                                  ...
                                                                  <td>{{$item->name}} </td>
                                                                  <td>{{$item->email}} </td>
                                                                  <td>{{__('translation.'.$item->role)}} </td>
                                                                  <td>{{$item->Parent->name ?? ''}} </td>
                                                                  <td>{{$item->City->name ?? ''}} </td>
                                                                  <td>
                                                                      <img src="{{getImage($item->image)}}" alt="" srcset="" width="150" height="150">
                                                                  </td>
                                                                <td class="btns">

                                                                        <a href="/dashboard/admins/{{$item->id}}/edit" type="button" class="btn btn-info   waves-effect waves-light m-b-5 btn-xs"> <i class="fa fa-pencil"></i> تعديل </a>
                                                                        <a href="{{route('admins.destroy',$item)}}" id="delete-btn" type="button" class="btn btn-danger   waves-effect waves-light m-b-5 btn-xs"> <i class="fa fa-times"></i> حذف </a>

                                                                    </td>


                                                                {{--  <td>
                                                                    <a href="/dashboard/cities/{{$item->id}}/edit">
                                                                        <i class="fa fa-pencil  m-r-10" style="color: #188ae2;">
                                                                        </i> تعديل</a>
                                                                </td>  --}}

                                                                {{--  <td>
                                                                     <a href="{{route('cities.destroy',$item)}}" id="delete-btn"  >
