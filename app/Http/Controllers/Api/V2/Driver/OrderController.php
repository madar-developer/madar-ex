...
class OrderController extends Controller
{
    ...
    public function index()
    {
        ...
        $countsByStatus = $statusCountsQuery
            ->select('status', \DB::raw('count(*) as total'))
            ->groupBy('status')
            ->pluck('total', 'status');

        $statusCounts = OrderStatus::orderBy('sort', 'asc')->get()->map(function ($status) use ($countsByStatus) {
            return [
                'id' => $status->id,
                'key' => $status->key,
                'name' => $status->name,
    ...
    public function InvoicesStatistics()
    {
        ...
        return Response()->json([
                'data' => [
                    'finance_not_collected_count' => $driver->Invoice()->where('orders.status', 'delivered')->where('orders.collected', 0)->count(),
                    'total_money_network' =>  ((clone $not_c)->where(function ($q){
                                            $q->where('payment_method_id', '=', '1')->orwhere('cash_type', '=', 'network_cash');
                                        })->where('cash_type', '=', 'network_cash')->sum('price')
                                                + (clone $not_c)->where('payment_method_id', '=', '1')->where('cash_type', '=', 'network')->sum('price')
                                                - (clone $not_c)->where(function ($q){
                    ...
                    'total_money_cash' =>  ((clone $not_c)->where(function($q){
                        $q->where('cash_type', '!=', 'network_cash')->orWhereNull('cash_type');
                    })->where('payment_method_id', '1')->sum('price') + (clone $not_c)->sum('cash_amount') ) , 
