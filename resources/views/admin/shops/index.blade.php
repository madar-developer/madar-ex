...
<!-- Page title -->
                                    ...
                                    <div class="add-btn">
                                        <a href="{{ url('/dashboard/shops/create') }}" type="button" class="btn btn-custom btn-rounded waves-effect waves-light w-md m-b-5"> <i class="fa fa-check"></i> اضافة </a>
                                    </div>
@endsection
@section('content')
                                                ...
                                                <div role="tabpanel" class="tab-pane " style="overflow-x: auto;">
                                                    <table id="datatable-buttons" class="table table-striped table-bordered">
                                                        ...
                                                        <tbody>
                                                            ...
                                                            <tr>
                                                                <td>{{$item->id}} </td>
                                                                <td> {{$item->the_name_of_the_store_or_company}} </td>
                                                                <td>{{$item->first_name}} </td>
                                                                <td>{{$item->last_name}}</td>
                                                                <td>{{$item->e_mail}}</td>
                                                                <td>{{$item->mobile_number}}</td>

                                                                <td> {{$item->Country->name_of_the_country}} </td>
                                                                <td> {{$item->TheCity->the_city_name}} </td>
                                                                <td> {{$item->TheCity->the_city_name}} </td>
                                                                <td>
                                                                <div class="btns">
                                                                    <button type="button" class="btn btn-success   waves-effect waves-light m-b-5 btn-xs"> <i class="fa fa-check"></i> Show</button>
                                                                    <a href="/dashboard/shops/{{$item->id}}/edit" type="button" class="btn btn-info   waves-effect waves-light m-b-5 btn-xs"> <i class="fa fa-pencil"></i> تعديل </a>
                                                                    <a href="{{route('shops.destroy',$item)}}" id="delete-btn" type="button" class="btn btn-danger   waves-effect waves-light m-b-5 btn-xs"> <i class="fa fa-times"></i> حذف </a>
                                                                </div>
                                                                </td>


                                                                {{--  <td>
                                                                    <a href="/dashboard/shops/{{$item->id}}/edit">
                                                                        <i class="fa fa-pencil  m-r-10" style="color: #188ae2;">
                                                                        </i> تعديل</a>
                                                                </td>  --}}

                                                                {{--  <td>
                                                                     <a href="{{route('shops.destroy',$item)}}" id="delete-btn"  >
